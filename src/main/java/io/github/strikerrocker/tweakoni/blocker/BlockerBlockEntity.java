...
public class BlockerBlockEntity extends BlockEntity implements Tickable {
    ...
    public void tick() {
        int tick = 0;
        if (tick++ % 20 == 0) {
            int x1 = world.getChunk(pos).getPos().getStartX() - 16;
            int z1 = world.getChunk(pos).getPos().getStartZ() - 16;

            int x2 = world.getChunk(pos).getPos().getEndX() + 16;
            int z2 = world.getChunk(pos).getPos().getEndZ() + 16;

            BoundingBox boundingBox = new BoundingBox(new BlockPos(x1, 0, z1), new BlockPos(x2, 256, z2));

            List<Entity> list = world.getEntities(MobEntity.class, boundingBox, (entity -> entity instanceof MobEntity && entity.isAlive()));
